...
import { Loader2, MailPlus, ChevronDownIcon, PlusIcon } from "lucide-react";
...
const OrganizationCard = (props: { session: Session | null }) => {
  ...
  const { data: activeOrganization, isFetching: isOrganizationFetching } = useOrganizationDetailQuery();
      ...
      <CardHeader>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <div className="flex items-center gap-1 cursor-pointer">
                <p className="text-sm">
                  <span className="font-bold"></span> {activeOrganization?.name || "Personal"}
                </p>
        ...
        <div className="flex items-center gap-2">
          <Avatar className="rounded-none">
            ...
            />
            <AvatarFallback className="rounded-none">{activeOrganization?.name?.charAt(0) || "P"}</AvatarFallback>
          </Avatar>
          <div>
            <p>{activeOrganization?.name || "Personal"}</p>
            <p className="text-xs text-muted-foreground">{activeOrganization?.members?.length || 1} members</p>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="flex gap-8 flex-col md:flex-row">
          <div className="flex flex-col gap-2 grow">
            ...
            <div className="flex flex-col gap-2">
              {activeOrganization?.members?.map((member) => {
                const isRemoving =
                  removeMemberMutation.isPending && removeMemberMutation.variables?.memberIdOrEmail === member.id;
          ...
          <div className="flex flex-col gap-2 grow">
            ...
            <div className="flex flex-col gap-2">
              ...
              {activeOrganization?.invitations?.filter((invitation) => invitation.status === "pending").length ===
                0 && (
                <motion.p

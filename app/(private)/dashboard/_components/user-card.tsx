...
import { Edit, Laptop, Loader2, LogOut, Phone, QrCode, ShieldCheck, ShieldOff, StopCircle } from "lucide-react";
        ...
        {session?.user.emailVerified ? null : (
          <Alert>
            ...
            <AlertDescription className="text-muted-foreground">
              Please verify your email address. Check your inbox for the verification email. If you haven't received the
        ...
        <div className="border-l-2 px-2 w-max gap-1 flex flex-col">
          ...
          {activeSessions
            ...
            .map((session) => {
              ...
              const isTerminating =
                revokeSessionMutation.isPending && revokeSessionMutation.variables?.token === session.token;
            ...
            <div className="flex gap-2">
              ...
              <Dialog open={twoFactorDialog} onOpenChange={setTwoFactorDialog}>
                ...
                <DialogContent className="sm:max-w-[425px] w-11/12">
                  ...
                  {session?.user.twoFactorEnabled ? (
                    <TwoFactorDisableForm onSuccess={() => setTwoFactorDialog(false)} />
                  ) : (
                    <TwoFactorEnableForm onSuccess={() => setTwoFactorDialog(false)} />
                  )}
                </DialogContent>
              </Dialog>
            </div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] w-11/12">
        ...
        <UpdateUserForm currentName={data?.user.name} onSuccess={() => setOpen(false)} />
      </DialogContent>
    </Dialog>
  );
}
